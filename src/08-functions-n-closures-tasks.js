...
function getPolynom(...args) {
  if (args.length === 0 || (args.some((item) => typeof item !== 'number'))) return null;
  const functions = [];

  for (let i = 0, len = args.length; i < len; i += 1) {
    const fn = (x) => args[i] * x ** (len - i - 1);
    functions.push(fn);
  }

  const resultFn = (x) => functions.reduce((acc, fn) => acc + fn(x), 0);
  return resultFn;
}
...
function logger(func, logFunc) {
  return (...args) => {
    const paramsString = args.map((param) => JSON.stringify(param)).join(',');

    logFunc(`${func.name}(${paramsString}) starts`);
    const result = func(...args);
    logFunc(`${func.name}(${paramsString}) ends`);
...
function partialUsingArguments(fn, ...args1) {
  return (...args2) => {
    if (fn.length === args1.length + args2.length) {
      return fn(...args1, ...args2);
    }
    return partialUsingArguments(fn, ...args1, ...args2);
  };
}
